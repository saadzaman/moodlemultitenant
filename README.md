# Iomad Multi‑Tenancy Implementation Guide
...
2. [Database Schema Extensions](#database-schema-extensions)
   - 2.1 [Company Table](#company-table)
   - 2.2 [Related Tables](#related-tables)
3. [Context Level: Company](#context-level-company)
4. [Local API Classes](#local-api-classes)
5. [UI Blocks & Selectors](#ui-blocks--selectors)
6. [Event Observers](#event-observers)
7. [Core File Patches](#core-file-patches)
8. [`iomad_check_course()` Utility](#iomad_check_course-utility)
...
_File: `local/iomad/db/install.xml`_
```xml
...
_File: `local/iomad/lib/company.php`_
```php
class company {
    public $id;
    public $context;
...
_File: `local/iomad/lib/user.php`_
```php
class company_user {
...
_File: `blocks/iomad_company_admin/lib/course_selectors.php`_
```php
...
_File: `blocks/iomad_company_admin/lib/user_selectors.php`_
```php
...
_File: `local/iomad/db/events.php`_
```php
...
_File: `local/iomad/classes/observer.php`_
```php
...
+   $params['id'] = SITEID; // redirect to front page
 }
...
### 7.2 `enrol/index.php`
**Customization:** Applies the same guard before loading the enrolment page, zeroing out the ID to force an error if the course is foreign.

```diff
 if (!iomad::iomad_check_course($id)) {
-   // no change in upstream
+   $id = 0; // block enrolment for other companies
 }
